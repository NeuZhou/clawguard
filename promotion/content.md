# ClawGuard — Promotion Materials

## 🇺🇸 English — Hacker News / Reddit / Twitter

### HN Title (Show HN)
**Show HN: ClawGuard – An immune system for AI agents (285+ threat patterns, 100% local)**

### HN Post Body
Hi HN,

I built ClawGuard — a security scanner and runtime protection layer for AI agents. Think of it as an immune system: it scans for prompt injection, data leakage, supply chain attacks, and 285+ threat patterns, all running 100% locally with zero cloud dependencies.
...
```bash
npx clawguard scan ./my-project/
npx clawguard sanitize "my API key is sk-proj-abc123..."
npx clawguard intent-check --intent "read the config" --action "rm -rf /"
```
...
2/ ClawGuard scans for 285+ threat patterns: prompt injection, data leakage, supply chain attacks, identity tampering. All running 100% locally.
...
6/ Try it: `npx clawguard scan ./your-project/`
...
所以我做了 **ClawGuard** — AI Agent 的免疫系统。
...
发送给 LLM 之前自动清洗敏感信息：
```
输入: "我的邮箱是 user@example.com，密钥是 sk-proj-abc123..."
...
```bash
$ clawguard intent-check --intent "查看天气" --action "rm -rf /home"
...
```bash
npx clawguard scan ./your-project/
npx clawguard sanitize "含敏感信息的文本"
npx clawguard check "可疑的消息内容"
```
...
### Title
**[P] ClawGuard: Open-source security scanner for AI agents — 285+ patterns, PII sanitizer, intent-action mismatch detection**
